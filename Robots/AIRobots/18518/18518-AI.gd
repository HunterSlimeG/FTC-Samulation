...
var nearestLaunch:= Vector3(0, 0, -0.4)
var inLaunch = true
	...
	if nav:
		...
		if not shooting:
			...
			if navCase==navCases.COLLECT:
				...
				navRotation = -Vector2($FlyWheel.global_position.x, $FlyWheel.global_position.z).angle_to_point(Vector2(targetPos.x, targetPos.z))+deg_to_rad(90)
	else:
